...
int LSM9DS1Class::begin()
{
  ...
  writeRegister(LSM9DS1_ADDRESS, LSM9DS1_CTRL_REG1_G, 0xb8); // 119 Hz, 2000 dps, 16 Hz BW 0x78 -> 0xd8 -> dd
  ...
  writeRegister(LSM9DS1_ADDRESS_M, LSM9DS1_CTRL_REG3_M, 0x80); // Continuous conversion mode
...
int LSM9DS1Class::readAccelerationMultiple(float& ax, float& ay, float& az, int n)
{
  int16_t accel_data[3];

  float temp_ax = 0, temp_ay = 0, temp_az = 0;
  for(int i=0;i<n;i++){
      readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_XL, (uint8_t*)accel_data, sizeof(accel_data));
      temp_ax += accel_data[0];
      temp_ay += accel_data[1];
      temp_az += accel_data[2];
  }

  float adiv = 2.0 / (32768.0 * n);
  ax = temp_ax * adiv;
  ay = temp_ay * adiv;
  az = temp_az * adiv;
...
int LSM9DS1Class::readAccelerationMultipleY(float& ay, int n)
{
  int16_t accel_data[1];

  float temp_ay = 0;
  for(int i=0;i<n;i++){
      readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_Y_XL, (uint8_t*)accel_data, sizeof(accel_data));
      temp_ay += accel_data[0];
  }
...
int LSM9DS1Class::readAccelerationGyroMultiple(float& ax, float& ay, float& az, float& gx, float& gy, float& gz, int n)
{

    float temp_ax = 0, temp_ay = 0, temp_az = 0, temp_gx = 0, temp_gy = 0, temp_gz = 0;
  ...
  for(int i=0;i<n;i++){
      readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_G, (uint8_t*)gyro_data, sizeof(gyro_data));
      temp_gx += gyro_data[0];
      temp_gy += gyro_data[1];
      temp_gz += gyro_data[2];
  }

//  int16_t* data = (int16_t*)malloc(sizeof(int16_t)*6*n);
//  readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_G, (uint8_t*)data, sizeof(data));
...
int LSM9DS1Class::accelerationAvailable()
{
  if (continuousMode) {
    ...
    int num_sample = readRegister(LSM9DS1_ADDRESS, 0x2F) & 63;
    if (num_sample) {
      return num_sample;
    }
  } else {
